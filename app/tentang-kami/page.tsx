...
import { getWebTeamMembers, getWebAboutContent, getWebHistoryTimeline, getWebMissionPoints, getWebLegalDocs } from '@/lib/web-queries';

export const revalidate = 300; // ISR: about-page content cached for 5 min

export default async function TentangKami() {
  const [teamMembers, about, timeline, mission, legalDocs] = await Promise.all([
    getWebTeamMembers(),
    getWebAboutContent(),
      ...
      <section className="bg-[#f4f6fb] py-16 px-6">
        <div className="max-w-[1060px] mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {legalDocs.map((l: any) => (
              <div key={l.title} className="bg-white rounded-xl p-5 border border-[#e2e8f0] border-l-4 border-l-[#83b64e]">
