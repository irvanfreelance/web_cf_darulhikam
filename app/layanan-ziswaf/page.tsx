...
export const revalidate = 300; // ISR: bank accounts cached for 5 min
            ...
            {banks.map((r: any) => (
              <div key={r.account_number} className="bg-[#f8fafc] border border-[#e2e8f0] rounded-xl p-5">
                {r.logo_url ? (
                  ...
                  <div className="inline-block bg-[#83b64e] text-white font-cabin font-bold text-[11px] py-1 px-2.5 rounded-md mb-3">
                    {r.bank_code || r.bank_name.slice(0, 3).toUpperCase()}
                  </div>
                )}
