...
export default function BottomNav() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 w-full max-w-md mx-auto bg-white border-t border-gray-100 flex justify-around px-2 py-2 pb-safe z-40 shadow-[0_-5px_20px_rgba(0,0,0,0.03)]">
      <Link href="/donasi" prefetch={true} className={cn("flex flex-col items-center flex-1 py-1", pathname === '/donasi' ? "text-teal-600" : "text-gray-400")}>
        <div className={cn(pathname === '/donasi' ? "bg-teal-50 px-4 py-1.5 rounded-full mb-1" : "mb-1.5")}>
          <Home size={20} className={pathname === '/donasi' ? "fill-teal-100" : ""} />
        </div>
      ...
      <Link href="/donasi/riwayat" prefetch={true} className={cn("flex flex-col items-center flex-1 py-1", pathname?.startsWith('/donasi/riwayat') ? "text-teal-600" : "text-gray-400")}>
        <div className={cn(pathname?.startsWith('/donasi/riwayat') ? "bg-teal-50 px-4 py-1.5 rounded-full mb-1" : "mb-1.5")}>
          <Heart size={20} className={pathname?.startsWith('/donasi/riwayat') ? "fill-teal-100" : ""} />
        </div>
