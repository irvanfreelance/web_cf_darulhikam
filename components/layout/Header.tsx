      ...
      <div className="flex items-center justify-between w-full">
        <div className="flex items-center gap-3">
          {logoUrl ? (
            ...
            <div className="w-10 h-10 bg-[#83b64e] rounded-xl flex items-center justify-center shadow-md shadow-brand-500/20">
          ...
          {(!logoUrl || title || subtitle) && (
            <div className="flex flex-col justify-center">
              {!logoUrl && (
                <span className="font-extrabold text-[#83b64e] text-lg leading-none tracking-tight font-sans">
                  {title || ngoName || "LAZ Darul Hikam"}
                </span>
              )}
              {(title && logoUrl) && (
                <span className="font-extrabold text-[#83b64e] text-lg leading-none tracking-tight font-sans">
        ...
        <Link href={session ? "/profil" : "/login"} className="w-10 h-10 bg-gray-50 rounded-full flex items-center justify-center text-gray-600 border border-gray-100 hover:bg-brand-50 hover:text-brand-600 transition-colors relative">
          <User size={20} />
          {session && (
