...
export default function CheckoutButton({ href, label }: { href: string; label: string }) {
  ...
  return (
    <button
      ...
      className={`w-full flex justify-center items-center gap-2 text-center text-white font-bold text-lg py-4 rounded-2xl shadow-lg transition-transform block ${loading ? 'bg-teal-500 shadow-none cursor-not-allowed' : 'cta-pulse bg-teal-600 shadow-teal-600/20 active:scale-[0.98] hover:bg-teal-700'}`}
    >
      {loading ? (
